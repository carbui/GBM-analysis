...
my_color <- scale_colour_gradientn(colours = myPalette(100)) 
```

#Load data and pre-process
##Load data from files after running Cellbender remotely at UPPMAX
```{r}
...
seurat_names <- c("seurat_1B", "seurat_1C", 
                  "seurat_1T", "seurat_2B", 
                  "seurat_2C", "seurat_2T", 
                  "seurat_3B", "seurat_3C", "seurat_3T")
...
##Visualize the doublets, subset to obtain only singlets, save
```{r}

# Path to save the processed objects
output_dir <- "~/Desktop/GBM_seurat/scDoubletFinder/"
...
##Filter with stricter parameters
```{r}
...
DotPlot(merged_seurat_filtered, features = markers_caro, assay= "RNA", cluster.idents = T, scale = T)+ 
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 8)) + my_color #palette
```
...
DietSeurat(selected_cells) #clean the new object from the processing done so far; then process it from the beginning
...
```
## Select gene sets and organism for Fgsea analysis
```{r}
library(msigdbr)
...
#and dotplot
ggplot(top10_UP_tumor_border, aes(x = NES, y = fct_reorder(pathway, NES), colour = pval)) + geom_point(aes(size = size)) + ggtitle("GO Tumor vs Border") + my_color
```
### tumor vs contra
```{r}
...
ggplot(top10_UP_tumor_contra, aes(x = NES, y = fct_reorder(pathway, NES), colour = pval)) + 
              ...
              ggtitle("GO Tumor vs Contra") + my_color
```
...
ggplot(top10_UP_border_contra, aes(x = NES, y = fct_reorder(pathway, NES), colour = pval)) + 
              geom_point(aes(size = size)) +
              ggtitle("GO Border vs Contra") + my_color
```
...
cluster_ids <- 0:5 # Replace 0:5 with the actual range of cluster IDs if changed
...
params <- list(
  ...
  min.pct = 0.1,
  min.diff.pct = 0.1,
  only.pos = FALSE, # depending if you only want upregulated or also downregulated genes
  assay = "RNA"
)
...
for (id in cluster_ids) {
  DGE_results[[paste0("DGE_cell_selection_", id)]] <- FindMarkers(
    sub_data,
    ident.1 = as.character(id),
    !!!params
  )
}

# save results
for (name in names(DGE_results)) {
write.csv(DGE_results[[name]], file = paste0(name, ".csv"), row.names = TRUE)
}
```
## Create a gene rank based on the gene expression fold change (SUBCLUSTERS)
```{r}
# Initialize a list to store gene ranks
gene_ranks <- list()

# Loop through each cluster ID
for (id in 0:5) {
  # Dynamically access DGE results and calculate gene ranks
  DGE_result <- DGE_results[[paste0("DGE_cell_selection_", id)]]
  gene_ranks[[paste0("gene_rank_", id)]] <- sort(setNames(DGE_result$avg_log2FC, rownames(DGE_result)), decreasing = TRUE)
}
